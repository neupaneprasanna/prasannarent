...
import { Loader2, Filter, Star, Info } from 'lucide-react';
                        ...
                        <div className="glass-card rounded-2xl p-6 sticky top-28">
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex gap-1">
                                    {[1, 2, 3, 4, 5].map((star) => (
                                        <button key={star} className="text-white/20 hover:text-[#fdcb6e] transition-colors" suppressHydrationWarning>
                            ...
                            <div className="mb-6">
                                <label className="flex items-center gap-3">
                                    <div className="relative w-10 h-5 bg-white/10 rounded-full">
                                        <div className="absolute left-0.5 top-0.5 w-4 h-4 rounded-full bg-[#6c5ce7] transition-transform" />
                                    </div>
                            ...
                            {/* Reset */}
                            <button
                                className="w-full py-2.5 text-xs text-white/40 hover:text-white/70 border border-white/10 rounded-xl hover:border-white/20 transition-all"
                                onClick={() => { setActiveCategory('All'); setPriceRange([0, 500]); }}
                                suppressHydrationWarning
                            >
                                Reset Filters
                            </button>
                            ...
                            <>
                                ...
                                <AnimatePresence mode="wait">
                                    {listings.length === 0 ? (
                                        ...
                                        <motion.div
                                            key={activeCategory + listings.length}
                                        ...
                                        >
                                            {listings.map((item, i) => (
                                                <RentalCard key={item.id} item={{
                                                    ...item,
                                                    priceUnit: item.priceUnit?.toLowerCase() || 'day',
                                                    owner: {
